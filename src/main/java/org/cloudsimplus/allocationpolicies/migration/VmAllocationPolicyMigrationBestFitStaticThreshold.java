 ...
 * detect host {@link #getUnderUtilizationThreshold() under} and {@link
 * #getOverUtilizationThreshold(Host) over} utilization.
 *
 * <p>It's a <b>Best Fit policy</b> which selects the Host having the most used amount of CPU MIPS
 * to place a given VM, <b>disregarding energy consumption</b>.
 *
...
public class VmAllocationPolicyMigrationBestFitStaticThreshold
  ...
  /**
   * Creates a VmAllocationPolicy. It uses a {@link #DEF_OVER_UTILIZATION_THRESHOLD default over
   * utilization threshold} and a {@link #DEF_UNDER_UTILIZATION_THRESHOLD default under utilization
   * threshold}.
   *
   ...
   * @param vmSelectionPolicy the {@link VmAllocationPolicyMigration#getVmSelectionPolicy() policy}
   *     that defines how VMs are selected for migration
   * @param overUtilizationThreshold {@link #setOverUtilizationThreshold(double) the over
   *     utilization threshold percent (between 0 and 1)}
   */
  public VmAllocationPolicyMigrationBestFitStaticThreshold(
      final VmSelectionPolicy vmSelectionPolicy, final double overUtilizationThreshold) {
   ...
   *
   * @param vmSelectionPolicy vmSelectionPolicy the {@link
   *     VmAllocationPolicyMigration#getVmSelectionPolicy() policy} that defines how VMs are
   *     selected for migration
   * @param overUtilizationThreshold {@link #setOverUtilizationThreshold(double) the over
   *     utilization threshold percent (between 0 and 1)}
   * @param findHostForVmFunction a {@link Function} to select a Host for a given Vm. Passing null
   *     makes the Function to be set as the default {@link #findHostForVm(Vm)}.
   ...
   *
   * <p>This method doesn't apply the additional filters from the super class. This way, Host
   * selection is performed ignoring energy consumption. However, all the basic filters defined in
   * the super class are ensured, since this method is called just after they are applied.
   *
   * @param vm {@inheritDoc}
  ...
  protected Optional<Host> findHostForVmInternal(final Vm vm, final Predicate<Host> predicate) {
    ...
    return getHostList().stream()
        .filter(predicate)
        .max(comparingDouble(Host::getCpuMipsUtilization));
  }
}

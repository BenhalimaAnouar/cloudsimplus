...
/**
 * An interface to be implemented by each class that represents a policy, used by a {@link
 * Datacenter}, to choose a {@link Host} to place or migrate a given {@link Vm} or {@link VmGroup}.
 *
 * <p>The VmAllocationPolicy uses Java 8+ Functional Programming to enable changing, at runtime, the
 * policy used to select a Host for a given {@link Vm} or {@link VmGroup}.
 *
...
public sealed interface VmAllocationPolicy
   ...
   * @return a {@link HostSuitability} to indicate if the Vm was placed into the host or not (if the
   *     Host doesn't have enough resources to allocate the Vm)
   * @see VmGroup
   */
  HostSuitability allocateHostForVm(Vm vm);

  /**
   * Tries to allocate a host for a each {@link Vm} or {@link VmGroup} into a given List. The method
   * should define how it will find a {@link Host#getSuitabilityFor(Vm) suitable Host} for each Vm
   * and then call {@link #allocateHostForVm(Vm, Host)} to actually create the VM inside the
   * selected Host.
   *
   ...
   *     containing a HostSuitability object for the last Host where each VM was tried to be
   *     created; or an empty set if the Datacenter has no hosts.
   * @see VmGroup
   */
   ...
   *
   * <ul>
   *   <li>the Vm is overloaded and the Host where the Vm is placed has enough capacity
   *   <li>the Vm is underloaded
   * </ul>
   *
   * The resource to be scaled is defined by the given {@link VerticalVmScaling} object.
   *
   * @param scaling the {@link VerticalVmScaling} object with information of which resource is being
  ...
  /**
   * Sets a {@link BiFunction} that selects a Host for a given Vm. This Function receives the
   * current VmAllocationPolicy and the {@link Vm} requesting to be placed. It then returns an
   * {@link Optional}{@code <Host>} that may contain a suitable Host for that Vm or not.
   *
   * <p>If no Function is set, the default VM selection method provided by implementing classes will
   * be used instead.
   *
   * @param findHostForVmFunction the {@link BiFunction} to set
   */
  VmAllocationPolicy setFindHostForVmFunction(
      BiFunction<VmAllocationPolicy, Vm, Optional<Host>> findHostForVmFunction);
  ...
  /**
   * Gets a map of optimized allocation for VMs, according to current utilization and Hosts under
   * and overloaded conditions. The conditions that will make a new VM placement map to be proposed
   * and returned is defined by each implementing class.
   *
   * @param vmList the list of VMs to be reallocated
   * @return the new vm placement map, where each key is a VM and each value is the host where such
   *     a Vm has to be placed
   */
  Map<Vm, Host> getOptimizedAllocationMap(List<? extends Vm> vmList);

  /**
   * Finds a suitable Host that has enough resources to place a given VM. Internally it may use a
   * default implementation or one {@link #setFindHostForVmFunction(BiFunction) set in runtime}.
   *
   * @param vm the vm to find a host for it
   * @return an {@link Optional} containing a suitable Host to place the VM; or an empty {@link
   *     Optional} if no suitable Host was found
   */
  Optional<Host> findHostForVm(Vm vm);

  /**
   * Checks if VM migrations are supported by this VmAllocationPolicy. Realize that even if the
   * policy allows VM migration, such operations can be dynamically enabled/disabled by the
   * Datacenter.
   *
   ...
   *
   * @return true if host search for VM placement must be performed in parallel; false if it's to be
   ...
   *
   * @param hostCountForParallelSearch the value to set (use {@link Integer#MAX_VALUE} to disable
   *     parallel search)
   * @return this VmAllocationPolicy
   */
